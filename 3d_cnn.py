...
def evaluate_cnn3d(learning_rate = 0.03, n_epochs = 30, batch_size = 1):
    ...
    test_model = theano.function(
        ...
        givens={x:test_set_x[index*max_batch:(index+1)*max_batch]})
    ...
    for cs in xrange(n_cases):
        ...
        leng=set_x.shape[0]
        n = leng//max_batch #to avoid runt out of memory because it happens 
        
        for index in range(n+1):
            aux=test_set_x[index*max_batch:(index+1)*max_batch,:]
            aux2=aux.shape.eval()[0]
            prediction, feature=test_model(index,aux2)
            overallPrediction=np.concatenate((overallPrediction,prediction),axis=0)
        sio.savemat(results_path + str(case)+'_prediction.mat',{'prediction':overallPrediction})                 
